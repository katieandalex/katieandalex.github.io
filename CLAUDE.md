...
- **Pages**: `index.html` (RSVP form only — this is the site landing page), `home.html` (the main/home page), `gifts.html` (no-registry gift/honeymoon-fund page), `venue.html`, `faqs.html`, `costumes.html` (past costumes timeline). Nav order across all pages: RSVP, Home, Gifts, Venue, FAQs, Past Costumes (RSVP is first, and RSVP is the root `index.html`).
